...
3. 点击「上传结算文件」，选择 `.xlsx` 文件；多工作表文件需明确勾选本次入库的一个或多个工作表。
...
- 最大 20 MB、每工作表最多 20,000 个数据行位置、100 列；可多选工作表，全部校验通过后按勾选顺序分别入库；中途失败保留已成功结果，重试仅提交剩余工作表。同业务键以后上传的工作表为准。
- 空行跳过；重复列名、空表头、无表头的额外单元格、无数据行拒绝。
- 日期支持 Excel 日期序号（含 1904 日期系统）、`YYYY-MM-DD`、`YYYY/M/D`、中文年月日、常见日期时间。日期区间不会自动合并或截断。
- 代理商与广告主去首尾空白后同时为空的行直接跳过（包括说明、合计等），不参与校验、入库或统计；仅其中一项为空仍报错，全部跳过则提示没有数据行。
- 代理商、广告主、任务名称必须非空，去除首尾空白，不设业务长度上限，匹配区分大小写；日期必须有效。同一工作表内重复的四字段及 fix 组合会被拒绝并提示源行号。
...
- 可选列 `fix`（小写，表头允许首尾空白）保存在 `extra_fields`，按原始字符串精确匹配，区分大小写和前导零；缺列与空字符串等价。原四项相同但 fix 不同的行可同时保存；相同 fix 再上传会更新，修改 fix 会新增并保留旧行。结算数不参与业务键。
- 仅在同一登录用户的数据内，按「代理商 + 广告主 + 日期 + 任务名称 + fix」匹配：存在则更新整行，不存在则新增。文件名、工作表和文件哈希不参与匹配。本次文件未包含的旧业务记录保留，其他登录用户的数据互不影响；不同广告主属于不同业务键（管理员上传也仅更新自己的范围）。
- 同一登录用户、同名文件、同一工作表重复上传复用原记录，名称区分大小写；文件名或工作表不同则新建记录。明细仍按业务键更新，未匹配的旧行保留，统计同步重算。列表按最近上传时间排序，默认隐藏 0 行记录，可勾选「显示空记录」。明细页隐藏额外字段，展示代理商 ID、广告主 ID 及按广告主及任务名称精确匹配的项目 ID；项目未匹配显示 0，CSV 仍保留额外字段。详情折叠展示每次上传时间、人员及新增/更新行数，旧历史无法还原的数量显示「历史未记录」。每次 CSV 归档均保留，明细和页面下载反映当前有效数据。
...
### 账户 ID 与项目映射（2026-09-16）

`settlement_rows.agency_id` 为代理商 ID，`advertiser_id` 为账户（广告主）ID。
`account_projects(mapping_id, project_id, project_name, advertiser_id, updated_at)` 仅保存上传任务对应的项目映射，真实项目 ID 唯一，允许匹配历史已删除项目；同一账户的不同任务可各自保存 project_id=0 占位记录。

配置 `Ding.OpenID` 后，上传通过 `Metadata.Endpoint`（默认 `http://172.16.3.25:8081/api/v1/execute_sql`）查询 ADT 账户及 ADN 项目。代理商、广告主名称与父子关系必须联合唯一匹配。已内置接口交接说明中的三组别名，Excel 原名称不变。查询零匹配时对应 ID 写 0：代理商可单独查到时保留其 ID，账户为 0；账户下无名称精确匹配的项目时，保存项目 ID 为 0、名称为上传任务名称的映射。多匹配返回具体行号和名称，需要核对源名称或在 `internal/metadata/client.go` 补充经确认的别名后重试；本版没有人工映射页面。接口故障、截断或入库失败时整次上传失败，映射与明细一起回滚。预览仍只校验 Excel，正式上传时解析 ID。

未配置 OpenID 时兼容原导入流程，新行 ID 为 0，更新已有行保留其 ID。历史行不会在启动时自动请求外部接口。新增字段允许 NULL，以兼容历史数据。

升级前停止旧实例并备份数据库，再使用当前源码构建的二进制。普通启动自动迁移字段和表；也可直接执行下列命令完成迁移和历史回填（不启动 HTTP）：

```bash
./bin/adn-report -f etc/config.yaml -sync-ids
```

回填处理全部历史明细的账户/任务组合，先解析全部组合，再以单个事务刷新账户 ID 并重建项目映射，移除旧版保存的无关项目；查询无匹配写 0，之后可再次回填；接口故障或多匹配则不写回填数据，但已完成的表结构迁移会保留。命令可重复执行，不修改结算金额、上传历史或 CSV。项目映射在对应账户及任务下次上传时更新。OpenID 仅保存在私有配置中，请勿提交。

项目同步按广告主 ID 和上传任务名称精确匹配，`account_projects.project_name` 保存任务名称；无匹配时保存项目 ID 0，同名多个项目则拒绝上传。`-sync-ids` 会重新解析全部历史账户/任务组合，并在同一事务中替换旧项目映射；升级请先停旧实例并备份，再执行回填。
